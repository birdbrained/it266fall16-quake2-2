...
/*
	...
	edict_t *self : client holding the sword
	vec3_t start : the place the player starts
	vec3_t aimdir : where the player is looking at
...
void sword_attack(edict_t *ent, vec3_t g_offset, int damage)
{
	...
	fire_sword(ent, start, forward, damage, VAMPIREKNIFE_KICK);
}
...
void Weapon_Sword_Fire(edict_t *ent)
{
	int damage;
	if (deathmatch->value)
		damage = VAMPIREKNIFE_DEATHMATCH_DAMAGE;
	else
		damage = VAMPIREKNIFE_NORMAL_DAMAGE;
...
void Poison_Arrow_Fire (edict_t *ent, vec3_t g_offset, int damage, int effect)
{
	// These are your vectors
	vec3_t	forward, right;
	vec3_t	start;
	...
	fire_poison_arrow (ent, start, forward, damage, 1000, effect);
...
void Weapon_Poison_Arrow_Fire (edict_t *ent)
{
	...
	if (deathmatch->value)
		...
		damage = 5;
	Poison_Arrow_Fire (ent, vec3_origin, damage, EF_BLASTER);
	ent->client->ps.gunframe++;
	ent->client->pers.inventory[ent->client->ammo_index] -= ent->client->pers.weapon->quantity;
}
...
void Weapon_PoisonArrows(edict_t *ent)
{
	...
	Weapon_Generic (ent, 4, 8, 52, 55, pause_frames, fire_frames, Weapon_Poison_Arrow_Fire);
}



// FYA railgun
void weapon_fya_railgun_fire (edict_t *ent)
{
	vec3_t		start;
	vec3_t		forward, right;
	vec3_t		offset;
	...
	VectorSet(offset, 0, 7,  ent->viewheight-8);
	P_ProjectSource (ent->client, ent->s.origin, offset, forward, right, start);
	fire_fya_rail (ent, start, forward, damage, kick);
...
void Weapon_Fya_Railgun (edict_t *ent)
{
	...
	Weapon_Generic (ent, 3, 18, 56, 61, pause_frames, fire_frames, weapon_fya_railgun_fire);
}



// ICE MISSILES
void Weapon_IceMissile_Fire (edict_t *ent)
{
	vec3_t	offset, start;
	vec3_t	forward, right;
	int		damage;
	...
	VectorSet(offset, 8, 8, ent->viewheight-8);
	P_ProjectSource (ent->client, ent->s.origin, offset, forward, right, start);
	fire_ice_missile (ent, start, forward, damage, 500, damage_radius, radius_damage);

	// send muzzle flash
	gi.WriteByte (svc_muzzleflash);
	gi.WriteShort (ent-g_edicts);
	gi.WriteByte (MZ_BLUEHYPERBLASTER | is_silenced); //MZ_ROCKET
	gi.multicast (ent->s.origin, MULTICAST_PVS);
